...
class MarkovChainEstimator:
    ...
    def calculate_aic(self, eta, k):
        aic = [eta[i] - 2 * (self.__num_states**k - self.__num_states**i) * (self.__num_states - 1) for i in range(0, k)]
        best_aic = aic.index(min(aic))
        return (aic, best_aic)

    def calculate_bic(self, eta, k, sample_size):
        bic = [eta[i] - (self.__num_states**k - self.__num_states**i) * (self.__num_states - 1) * math.log(sample_size) for i in range(0, k)]
    ...
    def run(self, data_path, max_possible_order, verbose=False):
        ...
        log_likelihood_ratio, log_likelihoods = self.calculate_log_likelihood_ratio(data, orders, return_log_likelihood=True)
